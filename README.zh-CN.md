...
**[快速开始](#快速开始) · [空转实验](#大家都跳过的那一步) · [为什么要 jev](#为什么要类型化决策又为什么要一个快的) · [工作方式](#工作方式) · [API](#api) · [常见问题](#常见问题) · [English](README.md)**
...
对一个"示例完全不需要决策模型也能跑"的库来说，这个名字是个合理的质疑。诚实的回答分两部分。

### 搜索空间本身就来自"决策是类型化的"

一个靠生成文本做决策的 agent，能交给调优循环的只有两样东西：一段 prompt，
以及每条样本一个"成没成"的标量。而一个分支都是类型化问题的 agent 交出的是结构，
而结构就是可搜索的东西的大部分。

| 循环能搜索什么 | 生成式 agent | 类型化决策 agent |
| :--- | :---: | :---: |
| 指令措辞 | 可以 | 可以 |
| 每个选项的描述文本 | 不行，选项不是一个声明出来的集合 | 可以 |
| 哪个决策先跑 | 不行，它是一整团 | 可以 |
| 每个决策能看到什么 | 不行，只有一个上下文 | 可以 |
| 什么时候弃权而不是硬答 | 没有可校准的概率可以卡阈值 | 可以 |
| 该磨**哪一个**选项、对着哪个竞争者磨 | 没有概率分布 | 可以 |
| 是哪个决策点输掉了这条样本 | 只有一个标量结果 | 可以 |

七个里有六个在没有类型化决策时根本不存在。这也是为什么它是一个 agent 库而不是 prompt 优化器：
被进化的单位是一个决策，而决策是有零件的。

最后两行是让循环"有的放矢"而不是乱改的原因。`confusions()` 读概率分布，
指出 agent 真正会互换的那一对选项，于是 `mutate_from_confusions` 去重写那条边界，
而不是把本来就对的地方换个说法。

### 而一次循环是几千个决策，所以单次决策的成本就是循环本身

跑一次 `experiments/jev_transfer.py` 是 14 代 × 3 个候选 × 96 条任务，加上 held-out，
约 4000 个决策——只为回答一个问题一次。是这个算术、而不是品味，
决定了业界的自进化循环普遍跳过 held-out 验证、只报训练集分数。

同一条网络、同一批样本、同一组选项、同一份描述文本下实测
（`jevbench/compare.py`，8 个易混意图共 160 条）：
...
请如实地读这张表：开箱即用时，决策模型是最快的，而且尾部远比其它模型紧；
同时它比最好的便宜生成模型**低 4.4 个点**。一个 4000 决策的循环，会把那个 p90 差距体感成小时。

所以这个库下的赌注很窄、也可证伪：**那 4.4 个点是 schema 的问题，不是模型的问题。**
那次对比里的规则是手写一次、之后再没被测量过的——而这正是本包存在要解决的状况。
`experiments/jev_transfer.py` 直接检验这个赌注：先在本地 0.5B 模型上免费把 schema 进化出来，
再只花 2N 次托管调用，比较初始 schema 与胜者。

这个实验也是 [后端](#后端) 那条建议唯一诚实的依据。
"本地进化、托管验证"如果成立的前提是"小模型喜欢的 schema 托管模型也喜欢"，
那这个前提必须被测量，而不是被假设。
...
那些搜索的是 prompt 文本和少样本示例，那只是这里七个维度中的一个。
另外六个只有在"决策是一个带具名选项和可校准概率的声明对象"时才存在：
选项描述、决策顺序、每个决策点的可见字段、弃权阈值、该对着哪个竞争者去磨、
以及是哪个决策点输掉了这条样本。而且运行结束给的是判定，不是一个最高分。

**必须有 Jev 或 System One 模型吗？**
不必，而这个名字仍然成立。这个库要求的是决策**被类型化**，整个搜索空间就是由它构成的。
jev 这类模型是提供类型化决策最便宜、延迟最低的方式，也正是它让一个几千决策的循环变得可行；
但 `LocalBackend` 和 `RuleBackend` 同样提供类型化决策。
见 [为什么要类型化决策](#为什么要类型化决策又为什么要一个快的)。
